...
BasicAnimation = function(p) { if( p === $_ ) return; {
	this.sum = this.loops = 0;
	this.ploter = p;
	this.values = new Array();
	{
		var _g1 = 0, _g = BasicAnimation.endValues.length;
		while(_g1 < _g) {
			var i = _g1++;
			this.values.push(0);
		}
...
BasicAnimation.__name__ = ["BasicAnimation"];
BasicAnimation.canvasDivId = null;
BasicAnimation.start = function(canvasDiv) {
	BasicAnimation.canvasDivId = canvasDiv;
	BasicAnimation.main();
}
BasicAnimation.main = function() {
	var p;
	var canvas = plotex.js.CanvasUtil.getCanvas(BasicAnimation.canvasDivId);
	p = new plotex.js.CanvasPloter(canvas.getContext("2d"));
	var t = new BasicAnimation(p);
}
BasicAnimation.prototype.values = null;
BasicAnimation.prototype.ploter = null;
BasicAnimation.prototype.timer = null;
BasicAnimation.prototype.time = null;
BasicAnimation.prototype.loops = null;
BasicAnimation.prototype.sum = null;
BasicAnimation.prototype.restart = function() {
	this.time = haxe.Timer.stamp();
	this.timer = new haxe.Timer(50);
	this.timer.run = $closure(this,"redraw");
}
BasicAnimation.prototype.redraw = function() {
	var t = haxe.Timer.stamp();
	this.ploter.clear();
	this.redrawBackground();
	var v;
	var stop = true;
	var w = 400 / BasicAnimation.endValues.length;
	var p = Math.min(1,(t - this.time) / BasicAnimation.duration);
	{
		var _g1 = 0, _g = this.values.length;
		while(_g1 < _g) {
			var i = _g1++;
			v = BasicAnimation.endValues[i] * p;
			this.redrawValue(w / 8 + i * w,w / 2,v,BasicAnimation.colors[i]);
			this.values[i] = v;
			if(v != BasicAnimation.endValues[i]) stop = false;
		}
	}
	if(stop) {
		this.timer.stop();
		haxe.Timer.delay($closure(this,"restart"),Std["int"](BasicAnimation.pause * 1000));
	}
	else this.sum += haxe.Timer.stamp() - t;
	++this.loops;
}
BasicAnimation.prototype.redrawBackground = function() {
	this.ploter.setColorFill({ r : 240, g : 240, b : 240});
	this.ploter.fillRect(0,0,400,400);
	this.ploter.setStroke(0.5,{ r : 200, g : 200, b : 200},70);
	{
		var _g = 0;
		while(_g < 10) {
			var i = _g++;
			this.ploter.line(0,i * 40,400,i * 40);
		}
	}
	this.ploter.stroke();
}
BasicAnimation.prototype.redrawValue = function(x,w,h,c) {
	var y = 400 - h - 5;
	this.ploter.setStroke(1,{ r : 0, g : 0, b : 0},90);
	this.ploter.setColorFill(c);
	this.ploter.fillRect(x,y,w,h);
	this.ploter.strokeRect(x,y,w,h);
	var darker = { r : c.r - 32, g : c.g - 32, b : c.b - 32};
	this.ploter.beginPath();
	this.ploter.setColorFill(darker);
	this.ploter.moveTo(x + w,y);
	this.ploter.lineUp(-h);
	this.ploter.lineTo(this.ploter.getX() + w / 2,this.ploter.getY() - w / 2);
	this.ploter.lineUp(h);
	this.ploter.lineRight(-w);
	this.ploter.lineTo(this.ploter.getX() - w / 2,this.ploter.getY() + w / 2);
	this.ploter.lineTo(x + w,y);
	this.ploter.closePath();
	this.ploter.fill();
	this.ploter.stroke();
	this.ploter.beginPath();
	this.ploter.lineTo(this.ploter.getX() + w / 2,this.ploter.getY() - w / 2);
...
BasicAnimation.prototype.__class__ = BasicAnimation;
...
BasicAnimation.colors = [{ r : 34, g : 64, b : 87},{ r : 55, g : 104, b : 140},{ r : 70, g : 134, b : 181},{ r : 34, g : 64, b : 87},{ r : 55, g : 104, b : 140}];
BasicAnimation.endValues = [100.0,150,200,50,300];
BasicAnimation.duration = 2.5;
BasicAnimation.pause = 1.;
